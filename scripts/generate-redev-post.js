...
- 글 구조: 개념 정의 → 왜 중요한가 → 분당·성남 상황과의 연결 → 독자들이 흔히 궁금해할 점 순서로 전개해.
- 반드시 \`## 소제목\` 형식(마크다운 H2)으로 3~5개의 소제목을 넣어 각 단락을 구분해. 소제목은 독자가 궁금해할 질문이나 핵심을 담은 구체적인 문구로 써(예: "## 재건축 초과이익환수제란 무엇인가", "## 분당 1기신도시는 왜 특별한가"). "서론/본론/결론" 같은 형식적 제목은 쓰지 마.
- 각 소제목 아래는 소제목 없이 자연스러운 서술형 문단으로 풀어써. 소제목 사이 본문은 2~4개 문단으로.
- 전체 분량은 2000~2800자 사이(소제목 포함). 각 소제목 섹션이 너무 짧지 않게 충실히 써.
...
PHOTO2: ([PHOTO2] 자리에 들어갈, PHOTO와는 다른 소재의 영어 사진 검색어 1~2단어)`;
    ...
    // 3단계: Pexels API 호출을 통해 어울리는 이미지 찾기 (대표 이미지 + 본문 중간 이미지)
    ...
    // 4-B단계: 본문 중간 [PHOTO2] 자리에 실제 이미지 삽입 (없으면 자리표시자만 제거)
    if (photo2) {
